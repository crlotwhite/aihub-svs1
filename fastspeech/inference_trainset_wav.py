...
def inference(files, model, vocoder, mel_spectogram, output_dir, config, desc):
    for file in tqdm.tqdm(files, desc=desc):
        ...
        for i, ali in enumerate(align):
            ...
            wave = vocoder.infer(mel_trans.unsqueeze(0).float().cpu(), sigma=1.0)
            wave = wave * MAX_WAV_VALUE
            wave = wave.cpu().detach().numpy().astype('int16')

            margin = (wave.shape[1] - (int(config.sampling_rate * end) - int(config.sampling_rate * start))) // 2
            if margin <= 0:
                wave = wave[0, :]
            else:
                wave = wave[0, margin:-margin]
...
if __name__ == "__main__":
    ...
    os.makedirs(output_dir, exist_ok=True)
    print('inference directory : ', output_dir)
    ...
    vocoder = torch.load(vocoder_file)['model']
